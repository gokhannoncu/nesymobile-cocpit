...
import { RecentQueriesTableShimmer } from './page-shimmer'
...
export function RecentQueriesTable({
  ...
  queries: MongoQueryRun[]
  loading?: boolean
  error?: string | null
  onReuse: (q: MongoQueryRun) => void
  onDelete: (q: MongoQueryRun) => Promise<void> | void
}) {
  const [selected, setSelected] = useState<MongoQueryRun | null>(null)
  const [deleteTarget, setDeleteTarget] = useState<MongoQueryRun | null>(null)
  const [deletingId, setDeletingId] = useState<string | null>(null)

  const requestDelete = (q: MongoQueryRun) => {
    setDeleteTarget(q)
  }
  ...
  if (loading) {
    return (
      <>
        ...
        <RecentQueriesTableShimmer rows={6} />
      </>
    )
  }
      ...
      <div className="overflow-x-auto rounded-xl border bg-background">
        <table className="w-full min-w-[980px] border-collapse text-sm">
          ...
          <tbody>
            {queries.length === 0 && (
              <tr>
                <td colSpan={7} className={cn(td, 'py-8 text-center text-muted-foreground')}>
...
export function RecentQueryDrawer({
  ...
  query: MongoQueryRun | null
  deleting?: boolean
  onClose: () => void
  onReuse: (q: MongoQueryRun) => void
  onDelete: (q: MongoQueryRun) => void
}) {
  const q = query
  return (
          ...
          <>
            ...
            <SheetBody className="h-[calc(100vh-96px)] space-y-6 overflow-y-auto px-5 py-5">
              ...
              <DrawerSection icon={FileText} title="Generated query" tone="orange">
                <MongoQueryEditor query={q.query} />
              </DrawerSection>
              ...
              <DrawerSection icon={History} title="Validation" tone="amber">
                <div className="space-y-1.5">
                  {(q.validation?.length ? q.validation : [{ detail: 'No validation history', status: 'warn' }]).map(
                    (v, i) => (
                      <div
                        key={i}
