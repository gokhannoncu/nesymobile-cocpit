...
export class MaestroExecutor extends EventEmitter {
  ...
  /**
   * Resolves as soon as the tracked node emits a DONE or FAIL step event,
   * without waiting for the Maestro process to fully exit.
   * This eliminates the 3-5 second Maestro shutdown delay between nodes.
   * The Maestro process is killed in the background after resolution.
   */
  async executeUntilStepDone(nodeId: string): Promise<MaestroResult> {
    this.startTime = Date.now();
    ...
    return new Promise<MaestroResult>((resolve) => {
      ...
      // Resolve early when the target step finishes
      const onStepEvent = (event: StepEvent) => {
        if (resolved) return;
        if (event.nodeId !== nodeId) return;
        if (event.type !== "done" && event.type !== "fail") return;

        resolved = true;
        this.off("step", onStepEvent);

        const result: MaestroResult = {
          exitCode: event.type === "fail" ? 1 : 0,
          signal: null,
          output: this.outputLines.join("\n"),
          duration: Date.now() - this.startTime,
        };

        // Kill the process in the background — don't block the caller
      ...
      this.on("step", onStepEvent);

      // Fallback: resolve when process exits (in case step event never fires)
      this.process.on("close", (code, signal) => {
        rl.close();
        rlErr.close();
