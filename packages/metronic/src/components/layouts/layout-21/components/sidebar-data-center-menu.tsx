    ...
    <>
      ...
      {MANAGEMENT_GROUPS.map((group) => (
        <div key={group.id}>
          ...
          >
            <AccordionMenuSub value={group.id}>
              <AccordionMenuSubTrigger value={`${group.id}-trigger`}>
              ...
              <AccordionMenuSubContent
                type="single"
                collapsible
                parentValue={`${group.id}-trigger`}
              >
                {group.items.map((item) => {
                  const locked =
                    LOCKED_OPERATION_PATHS.includes(item.path) && !operationsEnabled
